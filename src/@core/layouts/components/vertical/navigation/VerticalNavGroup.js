...
const VerticalNavGroup = (props) => {
  ...
  const conditionalIconColor = () => {
    if (skin === 'semi-dark' && theme.palette.mode === 'light') {
      ...
      return {
        color:
          parent && item.children ? theme.palette.secondary.light : theme.palette.secondary.main
      };
    } else
      return {
        color:
      ...
      <Fragment>
        <ListItem
          ...
          <ListItemButton
            ...
            <MenuItemTextWrapper
              sx={{ ...menuGroupCollapsedStyles, ...(isSubToSub ? { ml: 8 } : {}) }}>
              <Typography
                sx={{ ...conditionalArrowIconColor() }}
